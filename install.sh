...
#
# Flags:
#   --version <V>     same as DATAPRESS_VERSION
#   --bin-dir <DIR>   same as DATAPRESS_INSTALL_DIR
#   --help            show this help
...
main() {
    VERSION="${DATAPRESS_VERSION:-}"
    INSTALL_DIR="${DATAPRESS_INSTALL_DIR:-}"
    ...
    if [ -z "$VERSION" ]; then
        ...
        [ -n "$VERSION" ] || err "could not determine the latest version; set DATAPRESS_VERSION"
    fi
    ...
    say "Installing datapress $TAG"

    # ---- download + verify + extract ------------------------------------
    archive="${BIN_NAME}-${TAG}-${target}.tar.gz"
    base_url="https://github.com/${REPO}/releases/download/${TAG}"

    tmp="$(mktemp -d 2>/dev/null || mktemp -d -t datapress)"
    trap 'rm -rf "$tmp"' EXIT
...
usage() {
    ...
    cat <<EOF
Usage: install.sh [--version <V>] [--bin-dir <DIR>]
EOF
}
...
detect_target() {
    ...
    case "$_os" in
        ...
        *)
            err "unsupported OS: $_os (Windows users: use install.ps1)" ;;
    esac
}
...
latest_version() {
    ...
    # Trailing path component is the tag, e.g. .../tag/v0.4.4
    printf '%s\n' "${_eff##*/}"
}
