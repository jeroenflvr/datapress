...
mounted and behaves identically — examples below use it for brevity, but
...
## Recipes — end-to-end query bodies

Realistic combinations of every feature above. All bodies target
...
Populate a UI's "state" filter with the alphabetised distinct values
that actually appear in serious incidents.
...
Wide schema, narrow projection, ISO-string range, secondary sort. Fast
path on `state` (indexed) merged with SQL for the rest.
...
`ilike` for case-insensitive substring match, combined with a
not-null guard so geo-mapping clients don't see partial rows.
...
Frontend wants "show me the data" without ever scanning the whole
dataset. Hard `limit`, small `page_size`, deterministic sort so
...
All five aggregation ops on the same column, with explicit aliases so
the JSON keys match the UI's expected fields.
...
### R8. Pull a page as Arrow IPC into Polars

Same body as R4, but the client asks for binary Arrow. Pagination
metadata lives in `X-Page` / `X-Page-Size` response headers.

```bash
curl -X POST 'http://localhost:8000/api/v1/datasets/accidents/query?format=arrow' \
  -H 'Content-Type: application/json' \
  --output page.arrow \
  -d '{
    "columns": ["id","state","city","severity","start_time"],
...
Same predicates as the query, sent to `/count`. Returns just `{"count":N}`
— ideal for UI badges that update as the user adjusts filters.
...
The response has no total-count. Ask for one extra row to detect
"is there a next page?" without a second round-trip.
...
while True:
    body["page"] = page
    rows = httpx.post(url, json=body).raise_for_status().json()
