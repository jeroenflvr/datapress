<#
.SYNOPSIS
    datapress-cli installer for Windows.
...
.DESCRIPTION
    ...
    directory (no admin rights). Adds that directory to your user PATH if it is
    not already there.
...
.PARAMETER Version
    Version/tag to install (e.g. "0.4.11" or "v0.4.11"). Defaults to the latest
    release. Can also be set via $env:DATAPRESS_CLI_VERSION.

.PARAMETER InstallDir
    Directory to install into. Defaults to %LOCALAPPDATA%\datapress-cli\bin.
    Can also be set via $env:DATAPRESS_CLI_INSTALL_DIR.
...
[CmdletBinding()]
param(
    [string] $Version      = $env:DATAPRESS_CLI_VERSION,
    [string] $InstallDir   = $env:DATAPRESS_CLI_INSTALL_DIR,
    [switch] $NoModifyPath
)
...
switch ($arch) {
    ...
    default { Die "unsupported architecture: $arch. Try: cargo install datapress-cli" }
}
Write-Info "Detected platform: $target"

# ---- resolve install dir -------------------------------------------------
if ([string]::IsNullOrWhiteSpace($InstallDir)) {
    $InstallDir = Join-Path $env:LOCALAPPDATA 'datapress-cli\bin'
}
...
Write-Info "Installing datapress-cli $tag"

# ---- download + verify + extract ----------------------------------------
$archive = "datapress-cli-$tag-$target.zip"
$baseUrl = "https://github.com/$Repo/releases/download/$tag"

$tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("datapress-cli-" + [System.Guid]::NewGuid().ToString('N'))
...
try {
    ...
    Write-Info ""
    Write-Info "Installed datapress-cli to $dest"
}
finally {
...
if ($onPath) {
    Write-Info "$InstallDir is already on your PATH. Run: datapress-cli --version"
}
elseif ($NoModifyPath) {
    ...
    Write-Info "Open a new terminal (or restart it) and run: datapress-cli --version"
}
