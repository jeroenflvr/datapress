---
description: >-
  datap-rs-client: a standalone Python client for a DataPress server, backed by
  the native Rust client. Dict in, dict out, with optional pyarrow decoding.
---

# Python client (`datap-rs-client`)

A standalone Python client for a running DataPress server, backed by the native
[`datapress-client`](rust.md) Rust crate via PyO3. Requests are plain Python
dicts; responses come back as dicts. Structured queries can optionally be
decoded into a `pyarrow.Table`.

!!! note "Two Python clients"
    ...
    *run* a server — its bundled stdlib client is documented under
    [Python → Client](../python/client.md). Use `datap-rs-client` when you only
