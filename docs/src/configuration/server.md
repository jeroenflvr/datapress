...
| `force_lazy_above_mb`   | `0`         | `>0`: datasets whose local backing files exceed this many MiB are forced into `lazy` mode at startup. `0` disables. Local sources only (S3 never auto-forced); delta is measured by its parquet data files. |
